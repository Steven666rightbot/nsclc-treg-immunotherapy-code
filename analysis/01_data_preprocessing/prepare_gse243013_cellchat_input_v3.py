"""
Prepare GSE243013 CellChat input - single pass, buffered I/O.
"""
import os, gzip, random, json, tempfile
...
print(f"Strong: {len(strong_cols)} cols | Weak: {len(weak_cols)} cols")

# Step 3: Single pass - extract to temp files + count nnz
print(f"[{datetime.now()}] Step 3: Single pass extraction...")
print(f"Scanning 2B lines from {MTX_PATH}...")

strong_tmp = os.path.join(OUT_DIR, '_strong_tmp.mtx')
weak_tmp = os.path.join(OUT_DIR, '_weak_tmp.mtx')
...
with gzip.open(MTX_PATH, 'rt') as f:
    ...
    with open(strong_tmp, 'w', buffering=1024*1024) as f_s, open(weak_tmp, 'w', buffering=1024*1024) as f_w:
        for line in f:
            line_count += 1
            if line_count % 100000000 == 0:
                print(f"  {line_count//1000000}M lines...")
            
            # Fast parse: find two spaces
            parts = line.strip().split()
            col = int(parts[1])
            
            if col in strong_set:
                f_s.write(f"{parts[0]} {strong_col_map[col]} {parts[2]}\n")
                strong_nnz += 1
            if col in weak_set:
                f_w.write(f"{parts[0]} {weak_col_map[col]} {parts[2]}\n")
                weak_nnz += 1

print(f"  Done. Strong nnz: {strong_nnz} | Weak nnz: {weak_nnz}")

# Step 4: Write final matrices with headers
...
with open(strong_out, 'w') as f_out:
    ...
    f_out.write(f"{n_rows} {len(strong_cols)} {strong_nnz}\n")
    with open(strong_tmp, 'r') as f_in:
        f_out.write(f_in.read())

with open(weak_out, 'w') as f_out:
    f_out.write("%%MatrixMarket matrix coordinate real general\n")
    f_out.write(f"{n_rows} {len(weak_cols)} {weak_nnz}\n")
...
with open(os.path.join(OUT_DIR, 'sampling_info.json'), 'w') as f:
    json.dump({
        ...
        'strong_n': len(strong_cols), 'weak_n': len(weak_cols),
