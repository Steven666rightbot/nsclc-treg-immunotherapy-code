# =============================================================================
# scTenifoldKnk Virtual Knockout — Merged Treg (Weak + Strong) v3
# Strategy: top 2000 highly variable genes to reduce compute time
# =============================================================================
...
log_file <- file.path(OUTPUT_DIR, "log_v3.txt")
sink(log_file, split = TRUE, append = FALSE)

cat(rep("=", 60), "\n", sep = "")
cat("scTenifoldKnk Merged Treg KO v3 (top 2000 HVG)\n")
...
cat("Computing variance...\n")
expr_var <- apply(treg_all, 1, var)
top2000 <- names(sort(expr_var, decreasing = TRUE))[1:2000]

# Force-keep key genes and KO targets
targets <- c(TARGET_RECEPTORS, TARGET_TFS)
missing_targets <- targets[!(targets %in% top2000)]
if (length(missing_targets) > 0) {
  cat(sprintf("  Adding %d KO targets to top HVG: %s\n", length(missing_targets), paste(missing_targets, collapse=", ")))
  top2000 <- c(top2000, missing_targets)
}

missing_key <- KEY_GENES[!(KEY_GENES %in% top2000)]
if (length(missing_key) > 0) {
  cat(sprintf("  Adding %d key genes to top HVG: %s\n", length(missing_key), paste(missing_key, collapse=", ")))
  top2000 <- c(top2000, missing_key)
}

mat <- treg_all[top2000, ]
mat <- log1p(mat)
